...
class Board {
    ...
    void setBlock(char c, char r) {
        ...
        if (c == col && r == row) {
            // block position at the middle (NO reflection)
            setContent(c, r, BLOCKED);
            unblockedNum--;
        } else if (c != col && r != row) {
            // block position at the normal place (with 3 reflections)
            ...
            // block position at the middle line (with 1 reflection)
            setContent(col, row, BLOCKED);
            setContent(c, r, BLOCKED);
            unblockedNum -= 2;
        }
     ...
     * @return EMPTY if the game is finished but there is no winner / a tie.
      */
    PieceState getWinner() {
        ...
        if (!couldMove(RED) && !couldMove(BLUE))
            gameEnd = true;
        else if (getColorNums(RED) == 0 || getColorNums(BLUE) == 0)
            gameEnd = true;
        else if (getConsecJumpNums() >= CONSEC_JUMP_LIMIT)
            gameEnd = true;

        if (gameEnd) {
            if (getColorNums(RED) > getColorNums(BLUE))
                winner = RED;
            else if (getColorNums(BLUE) > getColorNums(RED))
                winner = BLUE;
            else winner = EMPTY;
        } else winner = null;
