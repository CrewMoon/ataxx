...
class Move {
    ...
    static boolean isClone(String location0, String location1) {
        ...
        if (location0 == null || location1 == null)
            return false;
		if (!location0.matches("[a-g][1-7]")
                || !location1.matches("[a-g][1-7]")) {
            return false;
        } else return !location1.equals(location0)
                    && (Math.abs(location0.charAt(0) - location1.charAt(0)) <= 1)
                    && (Math.abs(location0.charAt(1) - location1.charAt(1)) <= 1);
    }
    ...
    static boolean isJump(String location0, String location1) {
        ...
        if (!location0.matches("[a-g][1-7]")
            ...
            return false;
                ...
                && !isClone(location0, location1)
                && (Math.abs(location0.charAt(0) - location1.charAt(0)) <= 2)
                && (Math.abs(location0.charAt(1) - location1.charAt(1)) <= 2);
    }
